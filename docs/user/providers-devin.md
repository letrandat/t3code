...
T3 checks the binary manifest and checksum before launch. A failed or ended native run is not silently replaced. Open runs survive a T3 server restart or update and reconnect automatically on the next message; if a run cannot be reached, T3 reports it instead of starting a replacement. A reply that finishes while T3 is away is kept in the run's logs rather than streamed into the thread.
