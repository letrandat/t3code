...
Use **Stop** to request a soft cancel. Devin finishes a running tool and blocks the next tool call before returning to its waiting hook. Plain Escape retains its normal composer behavior. Send another message to continue the same native prompt. Send `/compact` while waiting to compact that prompt. `/clear` and model changes are not supported in this version.
